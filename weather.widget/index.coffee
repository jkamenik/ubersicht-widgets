options =
  city          : "Baltimore"       # default city in case location detection fails
  region        : "MD"              # default region in case location detection fails
...
style: """
  ...
  @font-face
    font-family Weather
    src url(weather-york.widget/icons.svg) format('svg')
...
command: "$(ps -o comm= $PPID | sed -e 's/UM-LM\^H/Ü/') weather-york.widget/get-weather \
                            \"#{options.city}\" \
                            \"#{options.region}\" \
                            #{options.units} \
